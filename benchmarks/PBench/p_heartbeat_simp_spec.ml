...
let ePing =
  [|
    (fun ?l:(tl = (true : [%v: int])) ->
      (allA, EPing true, [| EPong (trial == tl) |]));
  |]

let eShutDown = (allA, EShutDown true, [||])

val eStart : < > [@@gen]
val ePong : < trial : int > [@@obs]
val ePongLost : < trial : int > [@@obs]

let eStart = (allA, EStart true, [| EPing (trial == 1) |])
...
let ePongLost =
  [|
    (fun ?l:(tl = (v == 1 : [%v: int])) ->
      (allA, EPongLost true, [| EPing (trial == 2) |]));
    (fun ?l:(tl = (v == 2 : [%v: int])) ->
      (allA, EPongLost true, [| EPing (trial == 3) |]));
    (fun ?l:(tl = (v == 3 : [%v: int])) ->

...
module BasicIFC = struct
  ...
  let _init () = _ctx := { ctx = (init (), init ()); runtimeError = false }
...
let pushPublicHandler (msg : msg) =
  ...
  let depth = _exec ruleset_wrong_store_ab (Push (PublicV x)) in
...
let pushPrivateHandler (msg : msg) =
  let x, y =
    ...
    | _ -> _die [%here]
  in
  let depth = _exec ruleset_wrong_store_ab (Push (PrivateV (x, y))) in
  send ("stackDepth", [ mk_value_int depth ])

let popHandler (_ : msg) =
  let depth = _exec ruleset_wrong_store_ab Pop in
  send ("stackDepth", [ mk_value_int depth ])

let loadHandler (_ : msg) =
  let depth = _exec ruleset_wrong_store_ab Load in
  send ("stackDepth", [ mk_value_int depth ])

let storeHandler (_ : msg) =
  let depth = _exec ruleset_wrong_store_ab Store in
  send ("stackDepth", [ mk_value_int depth ])

let addHandler (_ : msg) =
  let depth = _exec ruleset_wrong_store_ab Add in
  send ("stackDepth", [ mk_value_int depth ])

let noopHandler (_ : msg) =
  let depth = _exec ruleset_wrong_store_ab Noop in
  send ("stackDepth", [ mk_value_int depth ])

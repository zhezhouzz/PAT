...
val updateAccounts : < tid : int ; name : int ; custid : int > [@@obs]
val updateSavings : < tid : int ; custid : int ; balance : int > [@@obs]
val updateChecking : < tid : int ; custid : int ; balance : int > [@@obs]
...
let commit ?l:(i = (true : [%v: int])) ?l:(j = (true : [%v: int])) =
  ( (starA (anyA - Commit (tid == i || cid >= j));
    ...
    starA
      (anyA
      - UpdateAccounts (tid == i)
      - SelectAccounts (tid == i)
      - UpdateSavings (tid == i)
      - SelectSavings (tid == i)
      - UpdateChecking (tid == i)
      - SelectChecking (tid == i)) )
...
let openAccountsReq (i : int) ?l:(n = (true : [%v: int]))
    ?l:(c = (true : [%v: int])) =
...
let amalgamateReq (i : int) ?l:(c0 = (true : [%v: int]))
    ?l:(c1 = (true : [%v: int])) =
  ( (allA;
    ...
    (BeginT (tid == i);
     ...
     starA
       (anyA
       - UpdateSavings (custid == c0)
       - UpdateChecking (custid == c0)
       - UpdateChecking (custid == c1));
     Commit (tid == i);
     AmalgamateResp true;
     allA) )
...
let balanceReq (i : int) (c : int) (bs : int) (bc : int)
    ?l:(n = (true : [%v: int])) =
  ( (allA;
     OpenAccountsReq (name == n && custid == c);
     allA),
    ...
    (BeginT (tid == i);
     ...
     allA;
     Commit (tid == i);
     (* TODO: the actual balance? *)
     BalanceResp (balance == bs + bc);
     allA) )

let balanceResp = (allA, BalanceResp true, allA)

let depositCheckingReq (i : int) (b : int) (c : int) ?l:(n = (true : [%v: int]))
    ?l:(a = (true : [%v: int])) =
  ( (allA;
     OpenAccountsReq (name == n && custid == c);
     allA),
...
let sendPaymentReq (i : int) (sb : int) (db : int) ?l:(sc = (true : [%v: int]))
    ?l:(dc = (true : [%v: int])) ?l:(a = (true : [%v: int])) =
  ( (allA;
...
let transactSavingsReq (i : int) (b : int) (c : int) ?l:(n = (true : [%v: int]))
    ?l:(a = (true : [%v: int])) =
  ( (allA;
     OpenAccountsReq (name == n && custid == c);
     allA),
...
let writeCheckReq (i : int) (c : int) ?l:(n = (true : [%v: int]))
    ?l:(a = (true : [%v: int])) =
  ( (allA;

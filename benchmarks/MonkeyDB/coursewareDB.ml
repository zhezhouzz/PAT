...
let do_getCourseEnrollments tid course_id =
  let msg =
    async ("getCourseEnrollments", [ mk_value_int tid; mk_value_int course_id ])
  in
  match msg.ev.args with _ :: _ :: _ :: _ :: args -> args | _ -> _die [%here]
...
let do_putCourseEnrollments tid course_id v =
  async
    ("putCourseEnrollments", [ mk_value_int tid; mk_value_int course_id ] @ v)
  ...
  if not is_student_registered then
    ...
    let remove_from_course course_id =
      ...
      let* _, _, course_enrollment_json =
        DB.table_async_get ~db:dbname ~tid ~table:"course_enrollments"
          ~key:course_id_str ()
      in
      ...
      | [ VCIntList student_ids ] ->
          ...
          DB.table_async_put ~db:dbname ~tid ~table:"course_enrollments"
            ~key:course_id_str
...
let deregisterStudentReqHandler (msg : msg) =
  let aux (student_id : int) =
    do_trans (fun tid ->
        if not (is_student_registered ~tid ~student_id) then false
        else
          ...
          let remove_from_course course_id =
            let student_ids =
              values_to_int_list (do_getCourseEnrollments tid course_id)
            in
            let student_ids' = List.filter (( <> ) student_id) student_ids in
            let _ =
              do_putCourseEnrollments tid course_id
                (int_list_to_values student_ids')
            in
            ()
          in
  ...
  let aux (course_id : int) =
    do_trans (fun tid ->
        if is_course_created ~tid ~course_id then false
          ...
          let _ =
            do_putCourseEnrollments tid course_id (int_list_to_values [])
          in
          true)
  in
  match msg.ev.args with
...
let deleteCourseReqHandler (msg : msg) =
  let aux (course_id : int) =
    do_trans (fun tid ->
        if not (is_course_created ~tid ~course_id) then false
        else
          ...
          let remove_student student_id =
            ...
            let _ =
              do_putStudentEnrollments tid student_id
                (int_list_to_values enrollments')
            in
            ()
          in
...
let async_enroll_student ~thread_id student_id course_id () =
  ...
  if (not student_registered) || not course_exists then
    ...
    in
    let newStudents =
      if List.mem student_id students then students else student_id :: students
    in
    let* () =
      DB.table_async_put ~db:dbname ~tid ~table:"course_enrollments"
        ~key:(string_of_int course_id)
  ...
  let aux (student_id : int) (course_id : int) =
    do_trans (fun tid ->
        ...
        then false
          ...
          let _ =
            do_putCourseEnrollments tid course_id
              (int_list_to_values newStudents)
          in
          true)
  in
  match msg.ev.args with
...
let getStudentEnrollmentsReqHandler (msg : msg) =
  let aux (student_id : int) =
    do_trans (fun tid ->
        if not (is_student_registered ~tid ~student_id) then None
        else
          let enrollments =
            values_to_int_list (do_getStudentEnrollments tid student_id)
          in
  ...
  | [ VConst (I student_id) ] -> (
      ...
      | None -> send ("getStudentEnrollmentsResp", [ mk_value_intList [] ]))
  | _ -> _die [%here]
...
let getCourseEnrollmentsReqHandler (msg : msg) =
  let aux (course_id : int) =
    do_trans (fun tid ->
        if not (is_course_created ~tid ~course_id) then None
        else
          let enrollments =
            values_to_int_list (do_getCourseEnrollments tid course_id)
          in
  ...
  | [ VConst (I course_id) ] -> (
      ...
      | None -> send ("getCourseEnrollmentsResp", [ mk_value_intList [] ]))
  | _ -> _die [%here]
